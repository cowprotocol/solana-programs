...
///
/// Wire format: `[discriminator=2, ..intent bytes]`, 1 +
/// [`EncodedOrderIntent::SIZE`]-bytes.
/// Required accounts:
...
impl<'a, A> InstructionInputParsing<'a, A> for CreateOrderInput<'a, A> {
    ...
    fn parse_body(instruction_data: &'a [u8], accounts: &'a [A]) -> Result<Self, ProgramError> {
        // Body (discriminator already stripped): exactly EncodedOrderIntent::SIZE
        // intent bytes.
...
pub mod fixtures {
    ...
    use crate::data::intent::{fixtures::sample_intent, EncodedOrderIntent};

    /// Number of accounts `CreateOrder` expects: owner, created_by, order PDA,
    /// and the system program.
    pub const NUM_ACCOUNTS: usize = 4;

    /// Canonical [`EncodedOrderIntent::SIZE`]-byte intent payload for a valid
    /// sell order. The owner is [`sample_intent`]'s; tests that need it read it
    /// back from the decoded intent rather than assuming a value.
    pub fn valid_intent_bytes() -> [u8; EncodedOrderIntent::SIZE] {
        (&EncodedOrderIntent::from(&sample_intent(Default::default()))).into()
    }
