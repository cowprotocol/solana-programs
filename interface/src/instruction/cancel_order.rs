...
/// this instruction creates it already cancelled; that creation only succeeds
/// if `created_by` signs, authorizing the rent it pays. When the order already
/// exists, no rent moves and `created_by` is unused.
///
...
///
/// Wire format: `[discriminator=11, ..intent bytes]`, 1 +
