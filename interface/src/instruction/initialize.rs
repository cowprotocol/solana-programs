...
///
/// `manager`, `reclaim_authority`, and `sweep_authority` are recorded verbatim
/// in the state PDA's data: the account authorized to add and remove solvers,
/// the account authorized to reclaim rent for buffers, and the account
/// authorized to place sweep orders. See
...
/// bytes), sweep_authority (32 bytes)]`, 97 bytes. Required accounts:
...
impl From<Initialize> for Instruction {
    fn from(builder: Initialize) -> Self {
        ...
        data.extend_from_slice(&builder.sweep_authority.to_bytes());
        Instruction {
...
impl<'a, A> InstructionInputParsing<'a, A> for InitializeInput<'a, A> {
    ...
    fn parse_body(instruction_data: &[u8], accounts: &'a [A]) -> Result<Self, ProgramError> {
        ...
        let (manager, reclaim_authority, sweep_authority) = array_refs![
            authorities,
            size_of::<Pubkey>(),
            size_of::<Pubkey>(),
            size_of::<Pubkey>()
        ...
        let sweep_authority = Pubkey::new_from_array(*sweep_authority);
...
mod tests {
    ...
    fn initialize_input_parses_valid_input() {
        ...
        let sweep_authority = pubkey_from_seed("sweep authority");
        ...
        let InitializeInput {
            ...
            sweep_authority: parsed_sweep_authority,
        ...
        assert_eq!(parsed_sweep_authority, sweep_authority);
    }
    ...
    fn instruction_data_has_expected_layout() {
        ...
        assert_eq!(&data[65..], &sweep_authority.to_bytes());
    }
    ...
    fn instruction_data_regression() {
        ...
        let sweep_authority = Pubkey::new_from_array([0x33; 32]);
