...
Note that deleting the order PDA is _not_ enough to cancel an order. In fact, if an order signature is available, the same order could always be created again until it expires.
