//! `CreateSelfOrder` instruction handler.
//!
...
//! authenticated by its owner: it's gated by the [`Role::SelfOrderAuthority`],
...
pub fn process_create_self_order(
    ...
    let self_order_authority: Pubkey =
        StateAccount::from_account(state_pda)?.authority(Role::SelfOrderAuthority);
    if !authority.is_signer() || authority.address() != &self_order_authority {
        return Err(SettlementError::UnauthorizedSelfOrder.into());
    }
...
mod tests {
    ...
    use cow_settlement_interface::instruction::create_self_order::SYSTEM_PROGRAM_ID;
    ...
    static SELF_ORDER_AUTHORITY: LazyLock<Address> =
        LazyLock::new(|| pubkey_from_seed("self-order authority"));
    ...
    fn intent_data(owner: Pubkey, created_on_chain: bool) -> Vec<u8> {
        ...
        self_order_data(&intent)
    }

    /// Accounts for a self order authorized by [`SELF_ORDER_AUTHORITY`],
    ...
    fn sanity_check_authority_at_expected_position() {
        ...
        assert_eq!(
            ...
            *SELF_ORDER_AUTHORITY,
            "the AUTHORITY variable doesn't point to the self-order authority"
        );
    }
    ...
    #[test]
    fn process_create_self_order_rejects_non_canonical_state_pda() {
    ...
    #[test]
    fn process_create_self_order_rejects_wrong_authority() {
    ...
    #[test]
    fn process_create_self_order_rejects_nonsigner_authority() {
        let mut accounts = base_accounts();
    ...
    #[test]
    fn process_create_self_order_rejects_owner_not_state_pda() {
        let mut accounts = base_accounts();
