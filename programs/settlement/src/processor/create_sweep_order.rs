//! `CreateSweepOrder` instruction handler.
//!
...
//! authenticated by its owner: it's gated by the [`Role::SweepAuthority`], and
//! the program forces the intent owner to be the state PDA so the order can
//! only ever sell funds controlled by the state PDA (notably the buffers).

use cow_settlement_interface::{
    data::state::StateAccount,
    instruction::{create_sweep_order::CreateSweepOrderInput, InstructionInputParsing},
...
pub fn process_create_sweep_order(
    program_id: &Address,
    ...
    let CreateSweepOrderInput {
        ...
        state_pda,
        order_pda,
    } = CreateSweepOrderInput::parse(instruction_data, accounts)?;

    check_state_pda(program_id, state_pda)?;

    // Only the sweep authority may create sweep orders.
    let sweep_authority: Pubkey =
        StateAccount::from_account(state_pda)?.authority(Role::SweepAuthority);
    if !authority.is_signer() || authority.address() != &sweep_authority {
        return Err(SettlementError::UnauthorizedSweep.into());
    }
...
mod tests {
    ...
    use cow_settlement_interface::instruction::create_sweep_order::fixtures::{
        sweep_order_data, NUM_ACCOUNTS,
    };
    use cow_settlement_interface::instruction::create_sweep_order::SYSTEM_PROGRAM_ID;
    ...
    static SWEEP_AUTHORITY: LazyLock<Address> =
        LazyLock::new(|| pubkey_from_seed("sweep authority"));
    ...
    fn base_init_args() -> StateInitArgs {
        StateInitArgs {
            ...
            sweep_authority: *SWEEP_AUTHORITY,
        }
    }
    ...
    fn intent_data(owner: Pubkey, created_on_chain: bool) -> Vec<u8> {
        ...
        sweep_order_data(&intent)
    }

    /// Accounts for a sweep order authorized by [`SWEEP_AUTHORITY`],
    ...
    fn sanity_check_authority_at_expected_position() {
        ...
        assert_eq!(
            ...
            *SWEEP_AUTHORITY,
            "the AUTHORITY variable doesn't point to the sweep authority"
        );
    }

    #[test]
    fn process_create_sweep_order_propagates_parse_error() {
        let mut data = intent_data(*STATE_PDA, true);
        data.push(0); // trailing byte triggers a parse error
        let mut accounts = fake_sequential_accounts::<NUM_ACCOUNTS>();
        assert_eq!(
            process_create_sweep_order(&PROGRAM_ID, &mut accounts, &data),
            Err(ProgramError::InvalidInstructionData),
        );
    }

    #[test]
    fn process_create_sweep_order_rejects_non_canonical_state_pda() {
        // `fake_sequential_accounts` puts the state PDA at an arbitrary
        // address, which isn't the canonical state PDA for this program.
        let mut accounts = fake_sequential_accounts::<NUM_ACCOUNTS>();
        assert_eq!(
            process_create_sweep_order(&PROGRAM_ID, &mut accounts, &intent_data(*STATE_PDA, true)),
            Err(SettlementError::StateAccountMismatch.into()),
        );
    }

    #[test]
    fn process_create_sweep_order_rejects_wrong_authority() {
        let mut accounts = base_accounts();
        accounts[AUTHORITY] = fake_signer(pubkey_from_seed("unrelated"));
        assert_eq!(
            process_create_sweep_order(&PROGRAM_ID, &mut accounts, &intent_data(*STATE_PDA, true)),
            Err(SettlementError::UnauthorizedSweep.into()),
        );
    }

    #[test]
    fn process_create_sweep_order_rejects_nonsigner_authority() {
        let mut accounts = base_accounts();
        // `fake_account`, unlike `fake_signer`, leaves the signer flag clear.
        accounts[AUTHORITY] = fake_account(*SWEEP_AUTHORITY);
        assert_eq!(
            process_create_sweep_order(&PROGRAM_ID, &mut accounts, &intent_data(*STATE_PDA, true)),
            Err(SettlementError::UnauthorizedSweep.into()),
        );
    ...
    #[test]
    fn process_create_sweep_order_rejects_owner_not_state_pda() {
        let mut accounts = base_accounts();
    ...
    #[test]
    fn process_create_sweep_order_rejects_intent_not_created_on_chain() {
        let mut accounts = base_accounts();
